    ...
    if (req.user.role === 'Admin' || req.user.role === 'Sales/Finance') {
      ...
      if (projectIds.length > 0) {
        try {
          ...
          console.log(`💰 Revenue this month: ₹${revenueResult} from ${projectIds.length} projects`);
        } catch (revError) {
          console.error('⚠️  Revenue calculation error:', revError.message);
          revenueResult = 0;
        }
      } else {
        console.log('⚠️  No projects found for revenue calculation');
      }
